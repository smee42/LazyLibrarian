                ...
                for book in resultxml:
                    ...
                    try:
                        ...
                        if (book.find('isbn13').text is not None):
                            ...
                            workId = BOOK_rootxml.find('./book/work/id').text

                            logger.debug(u"language: " + str(bookLanguage))
                            logger.debug(u"work id: " + str(workId))

                            SERIES_URL = 'http://www.goodreads.com/series/work.xml?id=' + str(workId) + '&' + urllib.urlencode(self.params)
                            logger.debug(u"Series URL: " + str(SERIES_URL))

                            try:
                                request = urllib2.Request(SERIES_URL)
                                if lazylibrarian.PROXY_HOST:
                                    request.set_proxy(lazylibrarian.PROXY_HOST, lazylibrarian.PROXY_TYPE)
                                request.add_header('User-Agent', USER_AGENT)
                                opener = urllib2.build_opener(SimpleCache.CacheHandler(".AuthorCache"), SimpleCache.ThrottlingProcessor(5))
                    ...
                    if not (re.match('[^\w-]', bookname)): #remove books with bad characters in title
                        if book_status != "Ignored":
                            ...
                            if seriesId and lazylibrarian.ADD_SERIES:
                                controlValueDict = {"SeriesID": seriesId}
                                newValueDict = {
                                    "AuthorName":   authorNameResult,
                                    "AuthorID":     authorid,
